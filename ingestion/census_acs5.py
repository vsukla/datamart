...
Subject table (fetched separately from /acs/acs5/subject):
  S2701_C03_001E - percent with health insurance coverage (pre-computed %)
...
# S2701_C03_001E is the pre-computed percentage (e.g. 92.9 for 92.9%)
# C27001 doesn't exist in the Census API; subject tables are in /acs/acs5/subject
SUBJECT_VARIABLES = ["NAME", "S2701_C03_001E"]


def _fetch_subject(year: int, geo_for: str) -> dict[str, float | None]:
    """Fetch pct_health_insured from ACS Subject tables; returns {fips: pct}."""
    url = f"{CENSUS_BASE_URL}/{year}/acs/acs5/subject"
    params = {
        "get": ",".join(SUBJECT_VARIABLES),
        "for": geo_for,
        "key": CENSUS_API_KEY,
    }
    delay = RETRY_BACKOFF
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            for row in rows[1:]:
                ...
                county = d.get("county", "")
                fips = state + county.zfill(3) if county else state
                val = d.get("S2701_C03_001E")
                try:
                    result[fips] = float(val) if val and str(val).strip() != SENTINEL else None
            ...
            if attempt == MAX_RETRIES:
                log.error("Failed subject fetch year=%s geo=%s: %s", year, geo_for, exc)
                raise
            log.warning("Attempt %d/%d failed for subject year=%s geo=%s: %s — retrying in %ds",
...
def _normalize(raw: dict, year: int, fips: str, geo_type: str, state_fips: str) -> tuple[dict, dict]:
    ...
    estimate = {
        ...
        "pct_health_insured": raw.get("_pct_health_insured"),
...
if __name__ == "__main__":
    ...
    for year in args.years:
        ...
        hi_state = _fetch_subject(year, "state:*")
        hi_county = _fetch_subject(year, "county:*")

        for raw in state_rows:
            fips = raw["state"].zfill(2)
            raw["_pct_health_insured"] = hi_state.get(fips)
            geo, est = normalize_state(raw, year)
            all_geos[geo["fips"]] = geo
            all_estimates.append(est)

        for raw in county_rows:
            fips = raw["state"].zfill(2) + raw["county"].zfill(3)
            raw["_pct_health_insured"] = hi_county.get(fips)
